...
window.__ModuleLoader__.load({id: 'asuna-ui-elements-v1', factory: (require) => {
  ...
  function viewStore() {
    ...
    async function refresh() {
      ...
      try {
        ...
        if (hash !== signature || value.error) { signature = hash; set({state: next, error: ''}); }
    ...
    function select(id) { if (id === selected) return; selected = id; ++request; signature = '';
      set({state: null, calls: [], error: ''}); connect(); void refresh(); }
    ...
    return {subscribe, getSnapshot, start, stop, refresh, select, command, stopHost, connect};
  }
  ...
  function nodesFor(state, calls) {
    ...
    for (const message of messages) {
      ...
      for (const step of steps) {
        if (isOutput(step)) {
          ...
          if (step.payload?.content || step.payload?.reasoning || step.status === 'error')
            process.push({key: `generation:${key}`, kind: 'generation', step,
      ...
      for (const [operation, group] of groups)
        process.push({key: `generation:${operation}`, kind: 'generation', calls: group, createdAt: group[0].createdAt});
  ...
  function Thinking({body, source, streaming, live}) {
    const [open, setOpen] = useState(Boolean(live));
    const lines = body.trimEnd().split(/\r?\n/);
    const summary = (streaming ? lines.at(-1) : lines[0]).replaceAll('**', '').slice(0, 120);
    return h(DisclosureRow, {icon: h(IconThinkOutline14, {size: 14}),
      title: `思考过程 · ${source}`, open, expandable: true, expandOnRowClick: true,
      onToggle: () => setOpen(!open), collapsedContent: h('small', null, summary)},
      h('div', {className: 'asuna-thinking-text'}, body));
  }
  function Contents({step, calls, message}) {
    const parts = (calls || []).flatMap(call => (call.parts || []).map((part, index) => ({...part, key: `${call.id}:${index}`, streaming: call.status === 'running'})));
    const structured = step?.payload?.phase === 'DECIDE' || calls?.some(call => call.phase === 'DECIDE') ||
      step?.type === 'execution.output' && (step?.payload?.content || '').trim().startsWith('{');
    const liveParts = () => h(React.Fragment, null, ...parts.map(part =>
      h('section', {className: 'asuna-part', key: part.key},
        part.field === 'reasoning_content' ? h(Thinking, {body: part.text, source: 'provider reasoning_content', streaming: part.streaming, live: !step && !message}) :
        structured ? code(part.text) : h(MarkdownText, {text: part.text, streaming: part.streaming, labels}))));
    if (step || message) {
      ...
      return h(React.Fragment, null,
        reasoning && h('section', {className: 'asuna-part'}, h(Thinking, {body: reasoning, source: 'DSH reasoning 回执'})),
        content && (structured ? code(content) : h(MarkdownText, {text: content, labels})));
    }
    return liveParts();
  }
  function Generation({node, conversation}) {
    ...
    const actor = message ? `${message.authorLabel || 'Asuna'} · SPEAK` : step?.label ||
      `${call?.lane === 'character' ? '角色脑' : '行动脑'} · ${call?.phase || ''}`;
    const error = step?.status === 'error' || call?.status === 'error' || message?.deliveryState === 'FAILED';
    const ongoing = !step && !message && !error;
    ...
    if (!message && (step?.payload?.phase === 'DECIDE' || call?.phase === 'DECIDE'))
      return h('div', {className: 'asuna-decision', 'data-message-key': node.key},
        h(DisclosureRow, {icon: h(StateDot, {state: error ? 'error' : ongoing ? 'ongoing' : 'done'}),
          title: `${actor} · 决策详情`, collapsedContent: h('small', null, clock(step?.createdAt || call?.createdAt)),
          open: decisionOpen, expandable: true, expandOnRowClick: true,
        ...
        error && h('p', {className: 'asuna-error-text'}, step?.payload?.finish_reason || '决策生成失败，部分内容已保留'));
    return h('article', {className: `asuna-message ${message ? 'asuna-assistant' : 'asuna-process'}`,
      'data-message-key': node.key},
      h('div', {className: 'asuna-meta'}, h(StateDot, {state: error ? 'error' : ongoing || pendingDelivery ? 'ongoing' : uncertainDelivery ? 'warning' : 'done'}),
        h('strong', null, actor), h('time', null, clock(step?.createdAt || call?.createdAt || message?.createdAt)),
        delivery && h('small', null, delivery)),
      h('div', {className: 'asuna-body'}, h(Contents, {step, calls, message})),
      error && h('p', {className: 'asuna-error-text'}, step?.payload?.finish_reason || '生成或发送失败，部分正文已保留'),
      h(Diagnostic, {step, calls, conversation}));
  }
  ...
  function Workbench() {
    ...
    const choose = item => { stick.current = true; lastScrollTop.current = 0; setAtBottom(true); setNotice(''); store.select(item.id); };
