...
Web 可选择本机及已配置的外部场景，查看原文、排队状态、角色与行动过程、失败 traceback。外部私聊只读；已配置 operator_sender_id 的群可由本机 owner 提交明确标注的群发言指令。启用集成 profile 后，检查器“集成”显示进程及日志。集成停止工具仍可停止服务并取消自动恢复。Web 服务自身退出会停止本次宿主；单独关闭浏览器不会。
...
可信本机 owner 入口（当前为 `local-dm/local-user`）默认可用已配置的集成与自我开发能力；角色决定是否委托，行动任务继承对应能力，行动脑决定是否调用工具。普通聊天不因此自动成为开发任务。聊天文字或旧 Web 提交字段不能授予能力；外部通道和 QQ 权限规则不变。集成开发目录与普通工作区、无网络沙箱仍各守原有资源边界。任务反馈保留任务能力；本机 owner 创建的计划沿现有机制继承集成授权，不扩大计划的开发能力继承。ADR-007 内部自我开发机会仅沿其原有开发授权，不自动取得集成能力。停止已启用服务使用已有集成工具；普通任务取消不等同撤销此前的持久启用。
