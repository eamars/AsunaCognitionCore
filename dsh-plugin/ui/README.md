...
当前 ADR-006 界面在 DSH 主页面的 Asuna 面板内，以固定版公开基础组件组合浅色三栏：左侧 Asuna 场景，中间真实正文、工具及折叠诊断，右侧沿用只读检查器。属于“原生基础组件复用＋Asuna业务组合”；完整 WorkspaceBrowser/Chat 未接入。`/asuna/` 现在转到同一 DSH 主页面。下文标明日期的 V1 验收记录是历史证据。
...
`start-asuna.cmd` 会让 DSH 自动在系统默认浏览器打开带 token 的本机地址，无需手动复制 token；进入后点击 DSH 侧栏的 **Asuna**。若系统浏览器未自动打开，再使用启动终端中 `dsh web:` 后面的完整地址。认证后也可直接打开 `http://127.0.0.1:8765/?asuna=1`；端口以启动输出为准。DSH 仍校验浏览器会话，并只监听本机 `127.0.0.1`。
...
- 左栏从现有 Asuna 场景/上下文投影；历史上下文只读。没有虚构 DSH workspace 或 session。
- 真实 MONOLOGUE/行动正文、思考及 SPEAK 按来源显示；生成中的实际片段原位增长，正式消息原位结算。网页折叠诊断只显示非内容元数据，不重复正文或思考；完整原始 provider 回包只留既有审计。工具与其它执行记录使用 DSH 基础组件折叠查看。
- 正文英文不单独强制字体；行内代码、代码围栏、JSON payload、检查器结构化值和模型 JSON 编辑框使用统一等宽字体。正文通过 DSH `MarkdownText` 安全呈现，不执行内容中的 HTML。
