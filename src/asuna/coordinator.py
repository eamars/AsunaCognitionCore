...
class Coordinator:
    ...
    def ingest(self, event: dict, *, persona='P1'):
        with self.lock:
            ...
            ep=self.store.put('episodes',{'_id':ep_id,'scene_id':scene['_id'],'scope_key':scene['scope_key'],'policy_epoch':scene['policy_epoch'],'source_event_id':event['event_id'],'episode_kind':event.get('episode_kind','external'),'state':'PREPARED','persona':persona,'manifest':manifest,'context':context,'system':system,'person_id':event['person_id'],'monologue_refs':[],**{k:event[k] for k in ('task_id','intent_revision') if k in event}},stream=ep_id)
                ...
                if ep['state']=='DECISION_ACCEPTED':
                    ...
                    if next_step=='delegate':
                        ...
                        if not self.store.db.tasks.find_one({'_id':task_id}):
                            self.store.put('tasks',{'_id':task_id,'request_key':task_id,'episode_id':ep_id,'scene_id':ep['scene_id'],'scope_key':ep['scope_key'],'requester_id':ep['person_id'],'policy_epoch':ep['policy_epoch'],'persona_revision':ep['manifest']['persona_revision'],'intent_revision':1,'goal':ep['decision']['goal'],'constraints':ep['decision']['constraints'],'raw_input_refs':['in-'+ep_id],'state':'READY','fencing_token':0,'tool_steps':0,'allowed_capabilities':['fixture.lookup','fixture.read_resource','fixture.stage_copy','fixture.commit_copy','fixture.run_checks']},stream=ep_id)
