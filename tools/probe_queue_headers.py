...
write_json(ev.root/'probe-plan.json',{'queue_hold_seconds':310,'boundary':'actual installed DSH pi-ai/OpenAI HTTP transport','substituted':'local deterministic upstream and tokenizer, not Gemma','required':'native stop and durable complete output after more than 300 seconds waiting for endpoint lock'})
...
def occupy():
    with EndpointLock(cfg['character']['base_url']):held.set();release.wait(310)
...
try:
    with DshLane(cfg,store,ev) as lane:
        ...
        assert elapsed>300 and value.finish_reason=='stop' and value.content=='LOCAL_TRANSPORT_FIXTURE'
        assert lane.proxy.calls and lane.proxy.calls[-1]['status']==200
        ev.record('transport.checked',{'elapsed_seconds':elapsed,'native_finish':value.finish_reason,'content':value.content})
    status='PASS'
