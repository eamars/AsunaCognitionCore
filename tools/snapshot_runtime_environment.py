...
ev=Evidence(ROOT/'reports'/('environment-v4-'+uuid.uuid4().hex[:10]))
...
current['adapter_runtime']={'bridge':ref(ROOT/'dsh-plugin/runtime-v2.ts'),
    ...
    'model_response_policy':'Durable local queue admission sends an SSE comment before waiting for the endpoint; it is not upstream success. Complete upstream response is persisted before model text reaches DSH.',
    'queue_transport_probe':ref(ROOT/'reports/queue-header-probe-cdb9228406/result.json'),
    'previous_version_correction':'Previous aggregate claimed a universal 1800s provider HTTP timeout, but ordinary runs still fell back to 300s unless explicitly overridden. Both lanes now resolve and freeze transport_read_timeout_seconds=1800 by default; older artifacts retain their actual configuration.',
